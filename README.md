...
- 10–500 毫秒/字实时调速，速度保存在服务器并在下次进入时自动恢复
...
- 每天北京时间 07:10 自动读取仓库 `daily-articles/YYYY-MM-DD.md`，并把英文文章导入对应的 `YYYY-MM` 月目录
...
每天生成的文章提交到 `main` 分支的 `daily-articles/YYYY-MM-DD.md`。LoveSuki 会在北京时间 07:10 直接请求当天这一个文件，不会遍历或补拉其他日期。若文章尚未生成或 GitHub 请求失败，服务器每 5 分钟重试一次，直到当天文章成功保存；跨过北京时间零点后，旧日期的重试会自动失效。成功后当天不再请求，服务重启后也会根据本地文章记录自动跳过。

同步请求会优先使用 IPv4，以避免部分服务器可以通过 `curl` 访问 GitHub、但 Node.js `fetch` 因 IPv6 网络不可用而失败的问题。同步状态可通过 `journalctl -u LoveSuki` 查看。

文章会按月份自动归入 `YYYY-MM` 目录，例如 `2026-09` 保存 2026 年 9 月的文章。部署新版后，过去已导入的 GitHub 日期文章也会自动迁移到对应月份目录。管理页右上角的“设置”中可以：

- 开启或关闭每天自动拉取；关闭后不会安排检查或失败重试
- 选择月目录内按日期顺序（从早到晚）或逆序（从晚到早）显示

设置与文章目录一起持久化保存在 `data/catalog.json`，包括每日同步开关、日期排序方式和逐字显示速度；服务重启和代码更新后仍会保留。自动导入的文章仍可以在管理页中编辑。
